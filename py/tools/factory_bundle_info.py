...
def _GetFactoryBundleInfo(board, factory_branch, repo_sync):
  ...
  ***
  ...
  """
  factory_board_files = BuildBoard(board).factory_board_files
  if not factory_board_files:
    logging.warn('No factory-board package found for %s on branch %s',
                 board, factory_branch)
    return None

  # Overlay is 3 levels before factory_board_files.
  overlay_relpath = os.path.dirname(os.path.dirname(
      os.path.dirname(factory_board_files)))
  bundle_readme_path = os.path.join(factory_board_files, 'bundle', 'README')
  bundle_readme_relpath = bundle_readme_path[len(overlay_relpath) + 1:]
...
def OutputBundleInfo(boards_branch_info, html_file, sync):
  ...
  if html_file is None:
    ...
    footnote = (
        ...
        'The file "chromeos-base/factory-board/files/bundle/README" '
