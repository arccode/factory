...
def MakeUpdateBundle(factory_image, md5sum, output):
  ...
  with MountPartition(factory_image, 1, BUNDLE_MOUNT_POINT,
    ...
    Spawn('echo %s > %s' % (pipes.quote(md5sum), pipes.quote(md5sum_file)),
...
def main():
  ...
  parser.add_argument('-m', '--md5sum', metavar='MD5SUM',
                      dest='md5sum',
                      help='content to put into /dev_image/factory/MD5SUM')
  parser.add_argument('-o', '--output', metavar='BZFILE',
                      dest='output',
                      default='factory.tar.bz2',
                      help='output bundle')
  args = parser.parse_args()
  MakeUpdateBundle(args.factory_image, args.md5sum, args.output)
