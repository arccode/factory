...
TEMPLATE = (
    re.compile(
        r"{(?P<operator>[\+\./;\?|!@])?(?P<varlist>[^}]+)}",
        re.UNICODE))
VAR = re.compile(
    r"^(?P<varname>[^=\+\*:\^]+)((?P<explode>[\+\*])|(?P<partial>[:\^]-?[0-9]+))?(=(?P<default>.*))?$",
    re.UNICODE)
...
def _tostring(varname, value, explode, operator, safe=""):
  ...
  if type(value) == type({}):
    ...
    if explode == "+":
      return ",".join([
          varname + "." + urllib.quote(key, safe) + "," + urllib.quote(
              value
              [key],
              safe)
          for key in keys])
      ...
      return ",".join(
          [
              urllib.quote(key, safe) + "," + urllib.quote(value[key], safe)
              for key in keys])
  else:
...
def _tostring_path(varname, value, explode, operator, safe=""):
  ...
  if type(value) == type([]):
    ...
    if explode == "+":
      return joiner.join([
          varname + "." + urllib.quote(key, safe) + joiner +
          urllib.quote(value[key], safe) for key in keys])
    elif explode == "*":
      return joiner.join(
          [
              urllib.quote(key, safe) + joiner + urllib.quote(value[key], safe)
              for key in keys])
    else:
      return ",".join(
          [
...
def expand(template, vars):
  def _sub(match):
    ...
    for varname, explode, partial in varnames:
      if varname in vars:
        value = vars[varname]
        # if not value and (type(value) == type({}) or type(value) == type([]))
        # and varname in defaults:
        ...
        continue
      retval.append(
          TOSTRING[operator](varname, value, explode, operator, safe=safe))
    if "".join(retval):
