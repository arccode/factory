...
from cros.factory.goofy import test_list_runner
from cros.factory.test import factory
from cros.factory.test import state
from cros.factory.test.test_lists import test_lists


class TestListRunnerTest(unittest.TestCase):
  ...
  def _testPickleSerializable(self, iterator):
    """A TestListRunner object should be pickleable.
    ...
    self.assertTrue(isinstance(deserialized_object,
                               test_list_runner.TestListRunner))
  ...
  def _AssertTestSequence(self, test_list, expected_sequence,
    ...
    iterator = test_list_runner.TestListRunner(
    ...
    def _MockedCheckRunIf(path):
      return test_list_runner.TestListRunner.CheckRunIf(
          iterator,
          path,
          test_arg_env={},
          get_data=_GetData)
...
class TestListRunnerBaseTest(TestListRunnerTest):
  """Test test_list_runner.TestListRunner.
  ...
  def testInitFromNonRoot(self):
    ...
    iterator = test_list_runner.TestListRunner(self.test_list.subtests[0])
    self._testPickleSerializable(iterator)
    self.assertListEqual([root_path], [frame.node for frame in iterator.stack])
...
class TestListRunnerParallelTest(TestListRunnerTest):
  TEST_LIST = """
    test_lists.FactoryTest(id='a', pytest_name='t_a')
    test_lists.FactoryTest(id='b', pytest_name='t_b')
    with test_lists.FactoryTest(id='G'):
...
class TestListRunnerActionOnFailureTest(TestListRunnerTest):
...
class TestListRunnerTeardownTest(TestListRunnerTest):
...
class TestListRunnerIterationTest(TestListRunnerTest):
