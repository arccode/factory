...
class ProbeFunctionTest(unittest.TestCase):
  ...
  def testProbeFunction(self):
    ...
    self.assertEqual(func(function.INITIAL_DATA), [{'result': 'FOO'}])
    self.assertEqual(func([{'other': 'BAR'}]),
                     [{'result': 'FOO', 'other': 'BAR'}])
    self.assertEqual(func([{'other': 'BAR1'},
                           {'other': 'BAR2'}]),
  ...
  def testProbeFunctionWithList(self):
    ...
    self.assertEqual(func(function.INITIAL_DATA),
