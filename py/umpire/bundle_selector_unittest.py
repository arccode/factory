...
from cros.factory.umpire.bundle_selector import ParseDUTHeader, SelectRuleset
...
class SelectRulesetTest(unittest.TestCase):
  def testDefault(self):
    config = yaml.load("""rulesets:
- bundle_id: 'default'
  note: 'Default bundle'
  active: true""")
    self.assertEqual('default', SelectRuleset(config, 'mac:aa:bb:cc:dd:ee:ff'))
...
- bundle_id: 'default'
  active: true""")
    self.assertEqual('ethernet_mac_matcher',
                     SelectRuleset(config, {'mac':'aa:bb:cc:dd:ee:ff'}))
    self.assertEqual('ethernet_mac_matcher',
                     SelectRuleset(config, {'mac.eth0':'aa:bb:cc:dd:ee:ff'}))
    self.assertEqual('wireless_mac_matcher',
                     SelectRuleset(config, {'mac.wlan0':'00:11:22:33:44:55'}))
...
- bundle_id: 'default'
  active: true""")
    ...
    self.assertEqual('default', SelectRuleset(config, dict(sn='SN001')))
...
- bundle_id: 'mac_matcher'
  ...
  match:
    ...
    self.assertEqual(None,
                     SelectRuleset(config, dict(sn='SN002')))
...
- bundle_id: 'sn_range_020_open'
  ...
  match:
    ...
    self.assertEqual('sn_range_001_005',
                     SelectRuleset(config, dict(sn='SN001')))
    self.assertEqual('sn_range_001_005',
                     SelectRuleset(config, dict(sn='SN003')))
    self.assertEqual('sn_range_001_005',
                     SelectRuleset(config, dict(sn='SN004')))

    # sn_range ['-', 'SN010'] matches sn <= 'SN010'.
    self.assertEqual('sn_range_open_010',
                     SelectRuleset(config, dict(sn='SN000')))
    self.assertEqual('sn_range_open_010',
                     SelectRuleset(config, dict(sn='SN006')))
    self.assertEqual('sn_range_open_010',
                     SelectRuleset(config, dict(sn='SN010')))
    self.assertEqual(None,
                     SelectRuleset(config, dict(sn='SN011')))

    # sn_range ['SN020', '-'] matches sn >= 'SN020'.
    self.assertEqual('sn_range_020_open',
                     SelectRuleset(config, dict(sn='SN020')))
    self.assertEqual('sn_range_020_open',
                     SelectRuleset(config, dict(sn='SN100')))
    self.assertEqual(None,
                     SelectRuleset(config, dict(sn='SN011')))
...
- bundle_id: 'mlb_sn_range_020_open'
  ...
  match:
    ...
    self.assertEqual('mlb_sn_range_001_005',
                     SelectRuleset(config, dict(mlb_sn='MLBSN001')))
    self.assertEqual('mlb_sn_range_001_005',
                     SelectRuleset(config, dict(mlb_sn='MLBSN003')))
    self.assertEqual('mlb_sn_range_001_005',
                     SelectRuleset(config, dict(mlb_sn='MLBSN004')))

    # mlb_sn_range ['-', 'MLBSN010'] matches sn <= 'MLBSN010'.
    self.assertEqual('mlb_sn_range_open_010',
                     SelectRuleset(config, dict(mlb_sn='MLBSN000')))
    self.assertEqual('mlb_sn_range_open_010',
                     SelectRuleset(config, dict(mlb_sn='MLBSN006')))
    self.assertEqual('mlb_sn_range_open_010',
                     SelectRuleset(config, dict(mlb_sn='MLBSN010')))
    self.assertEqual(None,
                     SelectRuleset(config, dict(mlb_sn='MLBSN011')))

    # mlb_sn_range ['MLBSN020', '-'] matches sn >= 'MLBSN020'.
    self.assertEqual('mlb_sn_range_020_open',
                     SelectRuleset(config, dict(mlb_sn='MLBSN020')))
    self.assertEqual('mlb_sn_range_020_open',
                     SelectRuleset(config, dict(mlb_sn='MLBSN100')))
    self.assertEqual(None,
                     SelectRuleset(config, dict(mlb_sn='MLBSN011')))
