...
def _SlotTimeOutFunc(manager):
  logging.debug('Timer is fired to check slots which are expired.')
  manager.delayed_call = None
  manager.CheckExpiredSession()

  if not manager.slots and not manager.wait_queue:
    logging.debug('No timer is fired.')
    return

  # Fire another timer for oldest slot now.
  joined_dict = copy.deepcopy(manager.slots)
  joined_dict.update(manager.wait_queue)
  oldest_slot = min(joined_dict, key=joined_dict.get)
  next_time = _SLOT_ALIVE_TIME - (time.time() - joined_dict[oldest_slot]) + 1
  manager.PrepareTimerForDeadSlot(next_time)
...
class DownloadSlotsManager(object):
  ...
  def _TryToRequestSlot(self):
    ...
    if len(self.slots) + len(self.wait_queue) < self.max_slots_num:
      ...
      self.PrepareTimerForDeadSlot()

      return (identity, 0)

    # All slots are occupied already so need to be put into queue.
    self.wait_queue[identity] = timestamp
    place = len(self.wait_queue)
    logging.debug('Slots are all occupied so need to wait in %d place.',
                  place)
  ...
  def _HeartBeat(self, identity):
    ...
    self.wait_queue[identity] = timestamp
    index = self.wait_queue.keys().index(identity)
    place = index + 1
  ...
  def CheckExpiredSession(self):
    now = time.time()
    self.slots = {slot: t for slot, t in self.slots.iteritems()
                  if now - t < _SLOT_ALIVE_TIME}
    self.wait_queue = collections.OrderedDict(
        (slot, t) for slot, t in self.wait_queue.iteritems()
        if now - t < _SLOT_ALIVE_TIME)
    self._CheckAvailableSlot()

  def PrepareTimerForDeadSlot(self, timeout=_SLOT_ALIVE_TIME):
    if self.delayed_call is None:
      self.delayed_call = reactor.callLater(timeout, _SlotTimeOutFunc, self)
