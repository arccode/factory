...
detect_status() {
  ...
  if [[ "${basename}" == ".wh."* ]]; then
    ...
    if [ -e "${TEMP_DIR}/${dirname}/.wh.${basename}" ]; then
      # found whiteout file
      echo "D"
    else
      echo "M"
...
get_file_path_of_deleted_file() {
  ...
  local relative_path="${whiteout_path#${TEMP_DIR}/}"
  local basename="$(basename "${relative_path}")"
  local dirname="$(dirname "${relative_path}")"
...
get_relative_path() {
  ...
  elif [[ "${file}" == "${TEMP_DIR}/"* ]]; then
    echo "${file#${TEMP_DIR}/}"
  else
    echo "${file} is not a file under ${WORKING_DIR} or ${TEMP_DIR}" >2
  fi
}
...
declare_status_list() {
  ...
  done < <(find "${TEMP_DIR}" '(' -type f -o -type l ')' -a -print0 2>/dev/null)
